...
public class TA {
    public static void main(String[] args) { 
        ...
        Scanner input = new Scanner(System.in); // deklarasi variabel scanner untuk input pengguna
        String[][] user = { //array 2 dimensi berisi username dan password
            {"manager1", "password1"},
            {"manager2", "password2"}
        };

        boolean isLoggedIn = false; //menentukan sudah berhasil login atau login gagal
        int percobaanLogin = 0; // menghitung percobaan login
        int maxPercobaanLogin = 3; // jumlah maksimal percobaan login 
        ...
        } while (!isLoggedIn && percobaanLogin < maxPercobaanLogin); // loop do while untuk input username dan password hingga login berhasil hingga jumlah percobaan kurang dari batas maksimal 

        if (isLoggedIn) { // login berhasil
            System.out.println("Selamat datang di aplikasi kami.");

            String[] sistem = {"Penggajian", "Data karyawan"}; //array 1 dimensi
            int inputSistem; 
            ...
            if (inputSistem >= 1 && inputSistem <= sistem.length) { //input sistem berada dalam panjang array jika valid maka akan membuka opsi yang dpilih
                if (inputSistem == 1) {
                    ...
                    while (sistemPeng) {
                        String[] divisi = {"Backoffice", "Operasional"}; //array 1 dimensi
                        double[][] gajiArray = { //array 2 dimensi
                            // Divisi: Backoffice
                        ...
                        if (divisiIndex >= 0) { // memeriksa indeks yang dipilih lebih besar atau sama 0
                            double gajipokok = gajiArray[divisiIndex][0];
                            double pajak = gajiArray[divisiIndex][1];
                            double tunjangan = gajiArray[divisiIndex][2]; // jika valid maka program akan berjalan sesuai input

                            System.out.println("Gaji kotor anda adalah = " + (gajipokok - (gajipokok * pajak) + (gajipokok * tunjangan))); // program menghitung dan mencetak gaji kotor karyawan dengan mengurangi nominal pajak dan menambahkan tunjangan

                            System.out.print("Masukkan Jam Lembur :");
                            int jamLembur = input.nextInt();

                            if (jamLembur > 0) { // jika jam lembur yang diinput lebih dari 0, maka program akan menghitung gaji lembur
                                int bonus = 20000;
                                double gajiLembur = jamLembur * bonus; // jam  lembur dikali bonus per jam lembur
